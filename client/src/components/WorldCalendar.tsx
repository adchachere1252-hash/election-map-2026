...
export default function WorldCalendar({ elections, onElectionClick }: WorldCalendarProps) {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-slate-900/80 backdrop-blur-sm border border-slate-700/50 rounded-xl overflow-hidden">
      ...
      {/* Calendar grid */}
      <div className="flex-1 grid grid-cols-7 auto-rows-fr overflow-y-auto touch-pan-y">
        {Array.from({ length: totalCells }, (_, i) => {
          ...
          return (
            <div
              key={i}
              className={`border-b border-r border-slate-800/50 p-0.5 sm:p-1 min-h-[44px] sm:min-h-[60px] relative transition-colors ${
              ...
              {isValidDay && (
                <>
                  ...
                  <div className="mt-0.5 space-y-0.5">
                    {dayElections.slice(0, window.innerWidth < 640 ? 2 : 3).map((election) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center gap-0.5 sm:gap-1 px-0.5 sm:px-1 py-0.5 rounded ${colors.bg} border ${colors.border} hover:brightness-125 transition-all group cursor-pointer`}
                        ...
                        >
                          <img
                            ...
                            className="w-3 h-2 sm:w-3.5 sm:h-2.5 rounded-sm object-cover flex-shrink-0"
                            onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }}
                          />
                          <span className="text-[8px] sm:text-[9px] text-slate-300 truncate group-hover:text-white">
                            {election.country.length > 6 ? election.countryCode : election.country}
                          </span>
                        </button>
                      );
                    })}
                    {dayElections.length > (window.innerWidth < 640 ? 2 : 3) && (
                      <span className="text-[8px] sm:text-[9px] text-slate-500 pl-0.5">
                        +{dayElections.length - (window.innerWidth < 640 ? 2 : 3)} more
                      </span>
                    )}
                  </div>
                </>
      ...
      {/* Footer legend */}
      <div className="p-2 sm:p-3 border-t border-slate-700/50 flex items-center justify-between">
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          {Object.entries(STATUS_COLORS).slice(0, 3).map(([status, colors]) => (
            <div key={status} className="flex items-center gap-1">
              <div className={`w-2 h-2 rounded-full ${colors.dot}`} />
              <span className="text-[9px] sm:text-[10px] text-slate-500">{status}</span>
            </div>
          ))}
        </div>
