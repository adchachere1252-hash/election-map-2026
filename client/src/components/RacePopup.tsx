...
function HousePopup({ race, onClose, onFocusMap }: { race: HouseRace; onClose: () => void; onFocusMap?: () => void }) {
  ...
  return (
    <div className="popup-enter">
      ...
      {/* Still-counting banner — shown when primary ballots are still being tabulated */}
      {race.status === "Primary" && race.notes && race.notes.toLowerCase().includes("counting") && (
        <div className="flex items-center gap-2 mb-3 px-3 py-2 rounded-lg bg-amber-950/60 border border-amber-700/40">
          <span className="relative flex h-2 w-2 flex-shrink-0">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2 w-2 bg-amber-500"></span>
          </span>
          <span className="text-xs text-amber-300 font-medium">⏳ Votes still counting — 2nd place uncalled
          </span>
        </div>
      )}
