const mysql = require('/home/ubuntu/election-map-2026/node_modules/.pnpm/mysql2@3.15.1/node_modules/mysql2/promise.js');
...
async function main() {
  ...
  console.log('[TX Watcher] Connected. Monitoring TX Senate and TX-18 every 30s...');
    ...
    try {
      ...
      if (sen) {
        ...
        if (pct > 0 && sen.pct_reporting !== lastTxSenPct) {
          console.log(`${now} [TX SENATE] ${pct}% reporting | C2: ${sen.candidate2_name || 'TBD'} ${sen.candidate2_vote_pct || '?'}% | Talarico ${sen.candidate1_vote_pct || '?'}%`);
